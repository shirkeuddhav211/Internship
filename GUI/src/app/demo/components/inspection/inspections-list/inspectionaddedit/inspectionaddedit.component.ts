...
export class InspectionaddeditComponent {
  ...
  isInspectionType1Required = true;
  ...
  checkStatus(status) {
    if (this.inspectionInfo.InspectionType4 != "") {
      return;
    }
    else if (status == 'AM') {
      this.isAm = true;
  ...
  setValidations(value: any) {
    ...
    if (value) {
      this.isInspectionType1Required = false;
    } else {
      this.isInspectionType1Required = true;
    }
  }
  ...
  isPmDisabled(): boolean {
    ...
    return this.inspectionInfo.InspectionType4 && this.inspectionInfo.AmPm === 'AM';
  }
